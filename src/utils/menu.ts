  ...
  return (
    dataSource.map((route) => {
        ...
        if (route.routes && route.routes.length > 0) {
          return  { ...route, access: 'isAuthenticatedMenu', component: !route.component ? null : require(`@/${albPath}`).default,routes:  getDynamicMenu(route.routes)};
        } else {
          // require 不能直接为参数 必须是一个字符串，否则 cannot find module
          return { ...route, access: 'isAuthenticatedMenu', component: !route.component ? null : require(`@/${albPath}`).default };
        }
      },
    ));
};
